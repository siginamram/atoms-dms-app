...
export class SlaGenerationAddComponent implements OnInit {
  ...
  constructor(
    ...
    this.slaForm = this.fb.group({
      ...
      slaGenerateDate : ['',Validators.required],
  ...
  generateSLA(): void {
    ...
    const payload = {
      ...
      slaGenerateDate :  formData.slaGenerateDate,
  ...
  onSubmit(): void {
    ...
    if (this.slaForm.valid) {
      ...
      const payload = {
        ...
        slaGenerateDate :  this.formatDate(new Date(this.slaForm.get('slaGenerateDate')?.value)),
        createdBy: parseInt(localStorage.getItem('UserID') || '0', 10),
        isActive: true,
        package: {
          basePackage: parseFloat(formData.basePackage),
