...
@Component({
  selector: 'app-client-videos-emergrncy-request',
 standalone: false,
  templateUrl: './client-videos-emergrncy-request.component.html',
  styleUrl: './client-videos-emergrncy-request.component.css',
   providers: [provideMomentDateAdapter(MY_FORMATS)],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ClientVideosEmergrncyRequestComponent implements OnInit {
...
ngOnInit(): void {
  this.route.queryParams.subscribe((params) => {
    ...
    if (clientIdParam) {
      this.clientId = parseInt(clientIdParam, 10);
      this.fetchClientDetails(this.clientId); // only if passed
    }
    ...
    this.contentData.filterPredicate = (data, filter) => {
  ...
  applyFilter(event: Event, column: string): void {
    ...
    this.contentData.filter = JSON.stringify(this.activeFilters); // Ensure Angular detects changes
  }
  fetchTableData(): void {
    ...
    this.operationsService
      .GetEmergencyVideoRequest(this.selectedDate)
      .subscribe({
        next: (response: any[]) => {
